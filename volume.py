...
def main(volume):
    ...
    for _ in range(1):
        ...
        for f,m in enumerate(krw_markets):
            vol = upbit.get_days_candles(m, count=1)[0]['candle_acc_trade_volume']
            ...
            check_remaining_candles_req(upbit)
            print(f,'/',len(krw_markets) ,m,'|',format(vol, '.2f'))
            # return 
            if volume < vol:
                result.append(m)
        wait(INTERVAL_MIN)
...
if __name__ == '__main__':
    ...
    print(len(a))
    # m = "KRW-DOGE"
    # a= Upbitpy().get_days_candles(m, count=1)[0]['candle_acc_trade_volume']
    # print(a)